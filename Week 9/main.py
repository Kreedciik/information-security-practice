...
def bytes2binary(byte_arr: bytes):
    ...
    '1111000010000000'
    '''
    return ''.join(format(byte, '08b') for byte in byte_arr)
...
def bin_xor(bin_str_1, bin_str_2):
    ...
    bin_str_1 = bin_str_1.rjust(max_length, '0')
    bin_str_2 = bin_str_2.rjust(max_length, '0')
    return ''.join('1' if bin_str_1[i] != bin_str_2[i] else '0' for i in range(max_length))
...
def f(right_half, sub_key):
    ...
    expanded = [right_half[i - 1] for i in E]

    xor_result = bin_xor("".join(expanded), sub_key)

    s = []
    for i in range(8):
        block = xor_result[i * 6:(i + 1) * 6]
        r = int(block[0] + block[5], 2)
        c = int(block[1:5], 2)
        s.extend(list(bin(S[i][r][c])[2:].zfill(4)))

    permuted = permute(s, P)
...
def encrypt_DES(key, message):
    ...
    permuted_block = ''.join(permute(message, IP))
    ...
    for i in range(16):
        l_new = right
        r_new = bin_xor(left, f(right, sub_keys[i]))
        left, right = l_new, r_new

    cipher = ''.join(permute((right + left), IP_inverse))
...
def are_random_tests_all_passes(n):
    ...
    for i in range(n):
        ...
        encrypted_message_by_crypto = cipher.encrypt(message)

        encrypted_message_by_my_DES = encrypt_DES(key, message)

        return encrypted_message_by_crypto == encrypted_message_by_my_DES
